...
%%%-------------------------------------------------------------------
-module(emqx_module_proto_stomp).
-author("root").
...
-module_type(#{create => on_module_create,
  description =>
  #{en =>
  <<69, 77, 81, 32, 88, 32, 83, 116, 111, 109, 112, 32,
    71, 97, 116, 101, 119, 97, 121>>,
    zh =>
    <<69, 77, 81, 32, 88, 32, 83, 116, 111, 109, 112, 32,
      230, 142, 165, 229, 133, 165, 231, 189, 145, 229,
      133, 179>>},
  destroy => on_module_destroy, name => stomp_protocol,
  params =>
  #{allow_anonymous =>
  #{default => true,
    ...
    #{default => <<103, 117, 101, 115, 116>>,
      ...
      title =>
      #{en =>
      <<68, 101, 102, 97, 117, 108, 116, 32, 80,
        97, 115, 115, 119, 111, 114, 100>>,
        zh =>
        <<233, 187, 152, 232, 174, 164, 229, 175,
          134, 231, 160, 129>>},
      type => string},
    default_username =>
    #{default => <<103, 117, 101, 115, 116>>,
      ...
      title =>
      #{en =>
      <<68, 101, 102, 97, 117, 108, 116, 32, 85,
        115, 101, 114, 110, 97, 109, 101>>,
        zh =>
        <<233, 187, 152, 232, 174, 164, 231, 148,
    ...
    #{default => 8192,
      ...
      title =>
      #{en =>
      <<77, 97, 120, 105, 109, 117, 109, 32, 66,
        111, 100, 121, 32, 76, 101, 110, 103,
        116, 104>>,
        zh =>
        <<230, 138, 165, 230, 150, 135, 228, 189,
          147, 230, 156, 128, 229, 164, 167, 233,
          149, 191, 229, 186, 166>>},
      type => number},
    max_header_length =>
    #{default => 1024,
      ...
      title =>
      #{en =>
      <<77, 97, 120, 105, 109, 117, 109, 32, 72,
        101, 97, 100, 101, 114, 32, 76, 101, 110,
        103, 116, 104>>,
        zh =>
        <<230, 156, 128, 229, 164, 167, 230, 138,
          165, 230, 150, 135, 229, 164, 180, 233,
          131, 168, 233, 149, 191, 229, 186,
          166>>},
      type => number},
    max_headers =>
    #{default => 10,
      ...
      <<77, 97, 120, 105, 109, 117, 109, 32, 72,
        ...
        <<230, 156, 128, 229, 164, 167, 230, 138,
          165, 230, 150, 135, 229, 164, 180, 233,
          131, 168, 230, 149, 176, 233, 135,
          143>>},
      type => number}},
  status => on_module_status,
  title =>
  #{en =>
  <<83, 116, 111, 109, 112, 32, 71, 97, 116, 101, 119,
    97, 121>>,
    zh =>
    <<83, 116, 111, 109, 112, 32, 230, 142, 165, 229, 133,
...
on_module_create(ModId, Config) ->
  ...
  Listeners = lists:map(fun ({Proto, ListenOn,
    Options}) ->
    emqx_stomp:start_listener({Proto, ListenOn,
      Options}),
    {Proto, ListenOn}
                        end,
    ListenerCfgs),
...
on_module_destroy(_ModId, #{listeners := Listeners}) ->
  [begin
     emqx_stomp:stop_listener({Proto, ListenOn, []})
   end
...
listener_name(tcp, ListenOn) -> {'stomp:tcp', ListenOn};
listener_name(ssl, ListenOn) -> {'stomp:ssl', ListenOn}.
