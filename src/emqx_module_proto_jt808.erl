...
%%%-------------------------------------------------------------------
-module(emqx_module_proto_jt808).
-author("root").
...
-module_type(#{create => on_module_create,
  description =>
  #{en =>
  <<69, 77, 81, 32, 88, 32, 74, 84, 47, 84, 56, 48, 56,
    32, 71, 97, 116, 101, 119, 97, 121>>,
    zh =>
    <<69, 77, 81, 32, 88, 32, 74, 84, 47, 84, 56, 48, 56,
      32, 230, 142, 165, 229, 133, 165, 231, 189, 145, 229,
      133, 179>>},
  destroy => on_module_destroy, name => jt808_protocol,
  params =>
  #{allow_anonymous =>
  #{default => true,
    description =>
    #{en =>
    <<74, 84, 47, 84, 56, 48, 56, 32, 71, 97,
      116, 101, 119, 97, 121, 32, 65, 108, 108,
      111, 119, 32, 65, 110, 111, 110, 121,
      109, 111, 117, 115>>,
      zh =>
      <<74, 84, 47, 84, 56, 48, 56, 32, 231, 189,
        145, 229, 133, 179, 230, 152, 175, 229,
        144, 166, 229, 133, 129, 232, 174, 184,
        229, 140, 191, 229, 144, 141, 231, 153,
        187, 229, 189, 149>>},
    order => 2, required => true,
    title =>
    #{en =>
    <<65, 108, 108, 111, 119, 32, 65, 110, 111,
      110, 121, 109, 111, 117, 115>>,
      zh =>
      <<229, 133, 129, 232, 174, 184, 229, 140,
        191, 229, 144, 141, 231, 153, 187, 229,
        189, 149>>},
    type => boolean},
    authentication =>
    #{default => <<>>,
      description =>
      #{en =>
      <<84, 104, 101, 32, 74, 84, 47, 84, 32, 56,
        48, 56, 32, 100, 101, 118, 105, 99, 101,
        32, 97, 117, 116, 104, 101, 110, 116,
        105, 99, 97, 116, 105, 111, 110, 32, 99,
        101, 110, 116, 114, 97, 108, 46, 32, 73,
        116, 32, 105, 115, 32, 110, 101, 99, 101,
        115, 115, 97, 114, 121, 32, 105, 102, 32,
        97, 108, 108, 111, 119, 95, 97, 110, 111,
        110, 121, 109, 111, 117, 115, 61, 102,
        97, 108, 115, 101>>,
        zh =>
        <<84, 104, 101, 32, 74, 84, 47, 84, 32, 56,
          48, 56, 32, 100, 101, 118, 105, 99, 101,
      ...
      title =>
      #{en =>
      <<65, 117, 116, 104, 101, 110, 116, 105,
        99, 97, 116, 105, 111, 110, 32, 85, 82,
        76>>,
        zh =>
        <<232, 174, 164, 232, 175, 129, 230, 156,
          141, 229, 138, 161, 229, 156, 176, 229,
          157, 128>>},
      type => string},
    dn_topic =>
    #{default =>
    <<106, 116, 56, 48, 56, 47, 37, 99, 47, 100,
      110>>,
      description =>
      #{en =>
      <<68, 111, 119, 110, 108, 105, 110, 107,
        32, 77, 101, 115, 115, 97, 103, 101, 32,
        84, 111, 112, 105, 99>>,
        zh =>
        <<228, 184, 139, 232, 161, 140, 230, 182,
          136, 230, 129, 175, 228, 184, 187, 233,
          162, 152, 230, 160, 188, 229, 188,
          143>>},
      order => 5,
      title =>
      #{en =>
    ...
    #{default => false,
      ...
      <<69, 110, 97, 98, 108, 101, 32, 83, 116,
        ...
        <<230, 152, 175, 229, 144, 166, 229, 188,
          128, 229, 144, 175, 231, 187, 159, 232,
          174, 161>>},
    ...
    #{default => 8192,
      description =>
      #{en =>
      <<77, 97, 120, 32, 70, 114, 97, 109, 101,
        32, 76, 101, 110, 103, 116, 104>>,
        zh =>
        <<229, 133, 129, 232, 174, 184, 230, 156,
          128, 229, 164, 167, 230, 138, 165, 230,
          150, 135, 233, 149, 191, 229, 186,
          166>>},
      order => 8,
      title =>
      #{en =>
    ...
    #{default => <<>>,
      ...
      <<84, 104, 101, 32, 74, 84, 47, 84, 32, 56,
        48, 56, 32, 100, 101, 118, 105, 99, 101,
        32, 114, 101, 103, 105, 115, 116, 114,
        97, 121, 32, 99, 101, 110, 116, 114, 97,
        108, 46, 32, 73, 116, 32, 105, 115, 32,
        110, 101, 99, 101, 115, 115, 97, 114,
        121, 32, 105, 102, 32, 97, 108, 108, 111,
        119, 95, 97, 110, 111, 110, 121, 109,
        111, 117, 115, 61, 102, 97, 108, 115,
        101>>,
        zh =>
        <<84, 104, 101, 32, 74, 84, 47, 84, 32, 56,
          48, 56, 32, 100, 101, 118, 105, 99, 101,
      ...
      <<82, 101, 103, 105, 115, 116, 114, 97,
        121, 32, 85, 82, 76>>,
        zh =>
        <<230, 179, 168, 229, 134, 140, 230, 156,
          141, 229, 138, 161, 229, 156, 176, 229,
          157, 128>>},
      type => string},
    up_topic =>
    #{default =>
    <<106, 116, 56, 48, 56, 47, 37, 99, 47, 117,
      112>>,
      description =>
      #{en =>
      <<85, 112, 108, 105, 110, 107, 32, 77, 101,
        115, 115, 97, 103, 101, 32, 84, 111, 112,
        105, 99>>,
        zh =>
        <<228, 184, 138, 232, 161, 140, 230, 182,
          136, 230, 129, 175, 228, 184, 187, 233,
          162, 152, 230, 160, 188, 229, 188,
          143>>},
      order => 6,
  ...
  title =>
  #{en =>
  <<74, 84, 47, 84, 56, 48, 56, 32, 71, 97, 116, 101,
    119, 97, 121>>,
    zh =>
    <<74, 84, 47, 84, 56, 48, 56, 32, 230, 142, 165, 229,
...
on_module_create(ModId, Config) ->
  ...
  Listeners = lists:map(fun ({Proto, ListenOn,
    Options}) ->
    emqx_jt808_app:start_listener({Proto,
      ListenOn,
      Options}),
    {Proto, ListenOn}
                        end,
...
on_module_destroy(_ModId, #{listeners := Listeners}) ->
  [begin
     emqx_jt808_app:stop_listener({Proto, ListenOn, []})
   end
...
listener_name(tcp, ListenOn) -> {'jt808:tcp', ListenOn};
listener_name(ssl, ListenOn) -> {'jt808:ssl', ListenOn}.
