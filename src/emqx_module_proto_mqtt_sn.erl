%%%-------------------------------------------------------------------
%%% @author root
%%% @copyright (C) 2020, <COMPANY>
%%% @doc
%%%
%%% @end
%%% Created : 29. 12月 2020 下午7:38
%%%-------------------------------------------------------------------
-module(emqx_module_proto_mqtt_sn).
-author("root").

-include("../include/emqx_module_spec_listener.hrl").

-export([on_module_create/2, on_module_destroy/2,
  on_module_update/4, on_module_status/2]).

-module_type(#{create => on_module_create,
  description =>
  #{en =>
  <<69, 77, 81, 32, 88, 32, 77, 81, 84, 84, 45, 83, 78,
    32, 71, 97, 116, 101, 119, 97, 121>>,
    zh =>
    <<69, 77, 81, 32, 88, 32, 77, 81, 84, 84, 45, 83, 78,
      32, 230, 142, 165, 229, 133, 165, 231, 189, 145, 229,
      133, 179>>},
  destroy => on_module_destroy, name => mqtt_sn_protocol,
  params =>
  #{enable_qos3 =>
  #{default => false,
    description =>
    #{en =>
    <<84, 111, 32, 99, 111, 110, 116, 114, 111,
      108, 32, 119, 104, 101, 116, 104, 101,
      114, 32, 97, 99, 99, 101, 112, 116, 32,
      97, 110, 100, 32, 112, 114, 111, 99, 101,
      115, 115, 32, 116, 104, 101, 32, 114,
      101, 99, 101, 105, 118, 101, 100, 32,
      112, 117, 98, 108, 105, 115, 104, 32,
      109, 101, 115, 115, 97, 103, 101, 32,
      119, 105, 116, 104, 32, 81, 111, 83, 32,
      45, 49>>,
      zh =>
      <<230, 152, 175, 229, 144, 166, 229, 164,
        132, 231, 144, 134, 32, 81, 111, 83, 32,
        228, 184, 186, 32, 45, 49, 32, 231, 154,
        132, 230, 182, 136, 230, 129, 175>>},
    order => 3, required => true,
    title =>
    #{en =>
    <<69, 110, 97, 98, 108, 101, 32, 81, 111,
      83, 32, 45, 49>>,
      zh =>
      <<229, 144, 175, 229, 138, 168, 32, 81,
        111, 83, 32, 45, 49>>},
    type => boolean},
    enable_stats =>
    #{default => false,
      description => #{en => <<>>, zh => <<>>}, order => 4,
      required => true,
      title =>
      #{en =>
      <<69, 110, 97, 98, 108, 101, 32, 83, 116,
        97, 116, 105, 115, 116, 105, 99>>,
        zh =>
        <<229, 144, 175, 231, 148, 168, 230, 149,
          176, 230, 141, 174, 231, 187, 159, 232,
          174, 161>>},
      type => boolean},
    idle_timeout =>
    #{default => 30000,
      description => #{en => <<>>, zh => <<>>}, order => 5,
      required => true,
      title =>
      #{en =>
      <<73, 100, 108, 101, 32, 84, 105, 109, 101,
        111, 117, 116>>,
        zh =>
        <<231, 169, 186, 233, 151, 178, 232, 182,
          133, 230, 151, 182, 230, 151, 182, 233,
          151, 180>>},
      type => number},
    listener =>
    #{common =>
    #{acceptors =>
    #{default => 8,
      description =>
      #{en =>
      <<84, 104, 101, 32, 110, 117,
        109, 98, 101, 114, 32, 111,
        102, 32, 97, 99, 99, 101,
        111, 112, 116, 111, 114, 115,
        44, 32, 73, 116, 32, 106,
        117, 115, 116, 32, 119, 111,
        114, 107, 115, 32, 102, 111,
        114, 32, 84, 67, 80, 47, 83,
        83, 76, 47, 68, 84, 76, 83,
        32, 115, 111, 99, 107, 101,
        116>>,
        zh =>
        <<230, 142, 165, 230, 148, 182,
          229, 153, 168, 230, 149, 176,
          233, 135, 143, 239, 188, 140,
          230, 137, 167, 232, 161, 140,
          32, 97, 99, 99, 101, 112,
          116, 32, 230, 147, 141, 228,
          189, 156, 231, 154, 132, 232,
          191, 155, 231, 168, 139, 230,
          149, 176, 233, 135, 143, 239,
          188, 140, 228, 187, 133, 229,
          175, 185, 32, 84, 67, 80, 47,
          83, 83, 76, 47, 68, 84, 76,
          83, 32, 83, 111, 99, 107,
          101, 116, 32, 230, 156, 137,
          230, 149, 136>>},
      order => 1, required => true,
      title =>
      #{en =>
      <<65, 99, 99, 101, 112, 116,
        111, 114, 115>>,
        zh =>
        <<230, 142, 165, 230, 148, 182,
          229, 153, 168, 230, 149, 176,
          233, 135, 143>>},
      type => number},
      active_n =>
      #{default => 100,
        description =>
        #{en =>
        <<83, 112, 101, 99, 105, 102,
          121, 32, 116, 104, 101, 32,
          123, 97, 99, 116, 105, 118,
          101, 44, 32, 78, 125, 32,
          111, 112, 116, 105, 111, 110,
          32, 102, 111, 114, 32, 116,
          104, 101, 32, 83, 111, 99,
          107, 101, 116>>,
          zh =>
          <<232, 174, 190, 231, 189, 174,
            32, 83, 111, 99, 107, 101,
            116, 32, 231, 154, 132, 32,
            123, 97, 99, 116, 105, 118,
            101, 44, 32, 78, 125, 32,
            233, 128, 137, 233, 161,
            185>>},
        order => 2, required => true,
        title =>
        #{en =>
        <<65, 99, 116, 105, 118, 101,
          78>>,
          zh =>
          <<65, 99, 116, 105, 118, 101,
            78>>},
        type => number},
      max_conn_rate =>
      #{default => 1000,
        description =>
        #{en =>
        <<65, 108, 108, 111, 119, 101,
          100, 32, 99, 111, 110, 110,
          101, 99, 116, 105, 111, 110,
          115, 32, 112, 101, 114, 32,
          115, 101, 99, 111, 110,
          100>>,
          zh =>
          <<232, 175, 165, 231, 155, 145,
            229, 144, 172, 229, 153, 168,
            230, 175, 143, 231, 167, 146,
            229, 133, 129, 232, 174, 184,
            231, 154, 132, 230, 156, 128,
            229, 164, 167, 232, 191, 158,
            230, 142, 165, 228, 184, 170,
            230, 149, 176>>},
        order => 3, required => false,
        title =>
        #{en =>
        <<77, 97, 120, 32, 67, 111,
          110, 110, 101, 99, 116, 105,
          111, 110, 32, 82, 97, 116,
          101>>,
          zh =>
          <<230, 156, 128, 229, 164, 167,
            232, 191, 158, 230, 142, 165,
            233, 128, 159, 231, 142,
            135>>},
        type => number},
      max_connections =>
      #{default => 1000000,
        description =>
        #{en =>
        <<84, 104, 101, 32, 109, 97,
          120, 105, 109, 117, 109, 32,
          99, 111, 110, 110, 101, 99,
          116, 105, 111, 110, 115, 32,
          111, 102, 32, 108, 105, 115,
          116, 101, 110, 101, 114>>,
          zh =>
          <<232, 175, 165, 231, 155, 145,
            229, 144, 172, 229, 153, 168,
            229, 133, 129, 232, 174, 184,
            231, 154, 132, 230, 156, 128,
            229, 164, 167, 232, 191, 158,
            230, 142, 165, 230, 149,
            176>>},
        order => 4, required => false,
        title =>
        #{en =>
        <<77, 97, 120, 32, 67, 111,
          110, 110, 101, 99, 116, 105,
          111, 110, 115>>,
          zh =>
          <<230, 156, 128, 229, 164, 167,
            232, 191, 158, 230, 142, 165,
            230, 149, 176>>},
        type => number},
      proxy_protocol =>
      #{default => false,
        description =>
        #{en =>
        <<69, 110, 97, 98, 108, 101,
          32, 116, 104, 101, 32, 80,
          114, 111, 120, 121, 32, 80,
          114, 111, 116, 111, 99, 111,
          108, 32, 86, 49, 47, 50, 32,
          105, 102, 32, 116, 104, 101,
          32, 69, 77, 81, 32, 88, 32,
          99, 108, 117, 115, 116, 101,
          114, 32, 105, 115, 32, 100,
          101, 112, 108, 111, 121, 101,
          100, 32, 98, 101, 104, 105,
          110, 100, 32, 72, 65, 80,
          114, 111, 120, 121, 32, 111,
          114, 32, 78, 103, 105, 110,
          120>>,
          zh =>
          <<229, 188, 128, 229, 144, 175,
            32, 80, 114, 111, 120, 121,
            32, 80, 114, 111, 116, 111,
            99, 111, 108, 32, 86, 49, 47,
            50, 239, 188, 140, 229, 166,
            130, 230, 158, 156, 32, 69,
            77, 81, 32, 88, 32, 233, 155,
            134, 231, 190, 164, 233, 131,
            168, 231, 189, 178, 229, 156,
            168, 32, 72, 65, 80, 114,
            111, 120, 121, 32, 230, 136,
            150, 32, 78, 103, 105, 110,
            120, 32, 229, 144, 142>>},
        order => 5, required => false,
        title =>
        #{en =>
        <<80, 114, 111, 120, 121, 32,
          80, 114, 111, 116, 111, 99,
          111, 108>>,
          zh =>
          <<229, 188, 128, 229, 144, 175,
            32, 80, 114, 111, 120, 121,
            32, 80, 114, 111, 116, 111,
            99, 111, 108>>},
        type => boolean},
      proxy_protocol_timeout =>
      #{default => 3000,
        description =>
        #{en =>
        <<84, 104, 101, 32, 116, 105,
          109, 101, 111, 117, 116, 32,
          102, 111, 114, 32, 97, 99,
          99, 101, 112, 116, 105, 110,
          103, 32, 112, 114, 111, 120,
          121, 32, 112, 114, 111, 116,
          111, 99, 111, 108, 44, 32,
          69, 77, 81, 32, 88, 32, 119,
          105, 108, 108, 32, 99, 108,
          111, 115, 101, 32, 116, 104,
          101, 32, 84, 67, 80, 32, 99,
          111, 110, 110, 101, 99, 116,
          105, 111, 110, 32, 105, 102,
          32, 110, 111, 32, 112, 114,
          111, 120, 121, 32, 112, 114,
          111, 116, 111, 99, 111, 108,
          32, 112, 97, 99, 107, 101,
          116, 32, 114, 101, 99, 101,
          118, 105, 101, 100, 32, 119,
          105, 116, 104, 105, 110, 32,
          116, 104, 101, 32, 116, 105,
          109, 101, 111, 117, 116>>,
          zh =>
          <<229, 164, 132, 231, 144, 134,
            32, 80, 114, 111, 120, 121,
            32, 80, 114, 111, 116, 111,
            99, 111, 108, 32, 231, 154,
            132, 232, 182, 133, 230, 151,
            182, 230, 151, 182, 233, 151,
            180, 239, 188, 140, 229, 166,
            130, 230, 158, 156, 232, 182,
            133, 232, 191, 135, 232, 175,
            165, 230, 151, 182, 233, 151,
            180, 230, 156, 170, 230, 148,
            182, 229, 136, 176, 32, 80,
            114, 111, 120, 121, 32, 80,
            114, 111, 116, 111, 99, 111,
            108, 32, 231, 154, 132, 230,
            138, 165, 230, 150, 135, 239,
            188, 140, 69, 77, 81, 32, 88,
            32, 229, 176, 134, 229, 133,
            179, 233, 151, 173, 232, 175,
            165, 32, 84, 67, 80, 32, 232,
            191, 158, 230, 142, 165>>},
        order => 6, required => false,
        title =>
        #{en =>
        <<80, 114, 111, 120, 121, 32,
          80, 114, 111, 116, 111, 99,
          111, 108, 32, 84, 105, 109,
          101, 111, 117, 116>>,
          zh =>
          <<80, 114, 111, 120, 121, 32,
            80, 114, 111, 116, 111, 99,
            111, 108, 32, 229, 164, 132,
            231, 144, 134, 232, 182, 133,
            230, 151, 182, 230, 151, 182,
            233, 151, 180>>},
        type => number},
      recbuf =>
      #{default => 2048,
        description =>
        #{en =>
        <<84, 104, 101, 32, 114, 101,
          99, 101, 105, 118, 101, 32,
          98, 117, 102, 102, 101, 114,
          32, 111, 102, 32, 115, 111,
          99, 107, 101, 116, 32, 40,
          111, 115, 32, 107, 101, 114,
          110, 101, 108, 41>>,
          zh =>
          <<83, 111, 99, 107, 101, 116,
            32, 230, 142, 165, 230, 148,
            182, 231, 188, 147, 229, 134,
            178, 229, 140, 186, 229, 164,
            167, 229, 176, 143, 32, 40,
            230, 147, 141, 228, 189, 156,
            231, 179, 187, 231, 187, 159,
            229, 134, 133, 230, 160, 184,
            231, 186, 167, 41>>},
        order => 8, required => false,
        title =>
        #{en =>
        <<82, 101, 99, 101, 105, 118,
          101, 32, 66, 117, 102, 102,
          101, 114>>,
          zh =>
          <<230, 142, 165, 230, 148, 182,
            231, 188, 147, 229, 134, 178,
            229, 140, 186>>},
        type => number},
      reuseaddr =>
      #{default => true,
        description =>
        #{en =>
        <<84, 104, 101, 32, 83, 79, 95,
          82, 69, 85, 83, 69, 65, 68,
          68, 82, 32, 102, 108, 97,
          103, 32, 102, 111, 114, 32,
          108, 105, 115, 116, 101, 110,
          101, 114>>,
          zh =>
          <<232, 174, 190, 231, 189, 174,
            32, 83, 111, 99, 107, 101,
            116, 32, 231, 154, 132, 32,
            83, 79, 95, 82, 69, 85, 83,
            69, 65, 68, 68, 82, 32, 230,
            160, 135, 232, 175, 134>>},
        order => 9, required => false,
        title =>
        #{en =>
        <<83, 79, 95, 82, 69, 85, 83,
          69, 65, 68, 68, 82, 32, 70,
          108, 97, 103>>,
          zh =>
          <<83, 79, 95, 82, 69, 85, 83,
            69, 65, 68, 68, 82, 32, 230,
            160, 135, 232, 175, 134>>},
        type => boolean},
      sndbuf =>
      #{default => 2048,
        description =>
        #{en =>
        <<84, 104, 101, 32, 115, 101,
          110, 100, 32, 98, 117, 102,
          102, 101, 114, 32, 111, 102,
          32, 115, 111, 99, 107, 101,
          116, 32, 40, 111, 115, 32,
          107, 101, 114, 110, 101, 108,
          41>>,
          zh =>
          <<83, 111, 99, 107, 101, 116,
            32, 229, 143, 145, 233, 128,
            129, 231, 188, 147, 229, 134,
            178, 229, 140, 186, 229, 164,
            167, 229, 176, 143, 32, 40,
            230, 147, 141, 228, 189, 156,
            231, 179, 187, 231, 187, 159,
            229, 134, 133, 230, 160, 184,
            231, 186, 167, 41>>},
        order => 7, required => false,
        title =>
        #{en =>
        <<83, 101, 110, 100, 32, 66,
          117, 102, 102, 101, 114>>,
          zh =>
          <<229, 143, 145, 233, 128, 129,
            231, 188, 147, 229, 134, 178,
            229, 140, 186>>},
        type => number}},
      protocol =>
      #{gateway_id =>
      #{default => 1,
        description => #{en => <<>>, zh => <<>>},
        order => 3, required => true,
        title =>
        #{en =>
        <<71, 97, 116, 101, 119, 97,
          121, 32, 73, 68>>,
          zh =>
          <<231, 189, 145, 229, 133, 179,
            73, 68>>},
        type => number},
        listen_on =>
        #{default =>
        <<48, 46, 48, 46, 48, 46, 48, 58, 49,
          49, 56, 52>>,
          description => #{en => <<>>, zh => <<>>},
          order => 1, required => true,
          title =>
          #{en =>
          <<76, 105, 115, 116, 101, 110,
            101, 114>>,
            zh =>
            <<231, 155, 145, 229, 144, 172,
              229, 156, 176, 229, 157,
              128>>},
          type => string},
        listener_type =>
        #{default => <<117, 100, 112>>,
          description => #{en => <<>>, zh => <<>>},
          enum => [<<117, 100, 112>>], order => 2,
          required => true,
          title =>
          #{en =>
          <<76, 105, 115, 116, 101, 110,
            101, 114, 32, 84, 121, 112,
            101>>,
            zh =>
            <<231, 155, 145, 229, 144, 172,
              231, 177, 187, 229, 158,
              139>>},
          type => string}},
      udp_options => #{}},
    password =>
    #{default => <<>>,
      description =>
      #{en =>
      <<84, 104, 101, 32, 112, 97, 115, 115, 119,
        111, 114, 100, 32, 111, 102, 32, 77, 81,
        84, 84, 45, 83, 78, 32, 100, 101, 118,
        105, 99, 101, 32, 116, 111, 32, 99, 111,
        110, 110, 101, 99, 116, 32, 116, 111, 32,
        69, 77, 81, 32, 88>>,
        zh =>
        <<77, 81, 84, 84, 45, 83, 78, 32, 232, 174,
          190, 229, 164, 135, 230, 142, 165, 229,
          133, 165, 229, 136, 176, 32, 69, 77, 81,
          32, 88, 32, 228, 184, 173, 231, 154, 132,
          229, 175, 134, 231, 160, 129>>},
      order => 2, required => false,
      title =>
      #{en => <<80, 97, 115, 115, 119, 111, 114, 100>>,
        zh => <<229, 175, 134, 231, 160, 129>>},
      type => string},
    predefinded =>
    #{default => #{},
      description =>
      #{en =>
      <<80, 114, 101, 45, 100, 101, 102, 105,
        110, 101, 100, 32, 84, 111, 112, 105, 99,
        115>>,
        zh =>
        <<233, 162, 132, 232, 174, 190, 228, 184,
          187, 233, 162, 152, 229, 136, 151, 232,
          161, 168>>},
      items =>
      #{schema =>
      #{topic =>
      #{default =>
      <<114, 101, 115, 101, 114, 118,
        101, 100>>,
        description =>
        #{en =>
        <<77, 81, 84, 84, 32, 84,
          111, 112, 105, 99>>,
          zh =>
          <<77, 81, 84, 84, 32,
            228, 184, 187, 233,
            162, 152>>},
        order => 1,
        title =>
        #{en =>
        <<77, 81, 84, 84, 32, 84,
          111, 112, 105, 99>>,
          zh =>
          <<77, 81, 84, 84, 32,
            228, 184, 187, 233,
            162, 152>>},
        type => string}},
        type => object},
      order => 6, required => false,
      title =>
      #{en =>
      <<80, 114, 101, 45, 100, 101, 102, 105,
        110, 101, 100, 32, 84, 111, 112, 105, 99,
        115>>,
        zh =>
        <<233, 162, 132, 232, 174, 190, 228, 184,
          187, 233, 162, 152, 229, 136, 151, 232,
          161, 168>>},
      type => array},
    username =>
    #{default =>
    <<109, 113, 116, 116, 45, 115, 110, 45, 49>>,
      description =>
      #{en =>
      <<84, 104, 101, 32, 117, 115, 101, 114,
        110, 97, 109, 101, 32, 111, 102, 32, 77,
        81, 84, 84, 45, 83, 78, 32, 100, 101,
        118, 105, 99, 101, 32, 116, 111, 32, 99,
        111, 110, 110, 101, 99, 116, 32, 116,
        111, 32, 69, 77, 81, 32, 88>>,
        zh =>
        <<77, 81, 84, 84, 45, 83, 78, 32, 232, 174,
          190, 229, 164, 135, 230, 142, 165, 229,
          133, 165, 229, 136, 176, 32, 69, 77, 81,
          32, 88, 32, 228, 184, 173, 231, 154, 132,
          231, 148, 168, 230, 136, 183, 229, 144,
          141>>},
      order => 1, required => false,
      title =>
      #{en => <<85, 115, 101, 114, 110, 97, 109, 101>>,
        zh =>
        <<231, 148, 168, 230, 136, 183, 229, 144,
          141>>},
      type => string}},
  status => on_module_status,
  title =>
  #{en =>
  <<77, 81, 84, 84, 45, 83, 78, 32, 71, 97, 116, 101,
    119, 97, 121>>,
    zh =>
    <<77, 81, 84, 84, 45, 83, 78, 32, 230, 142, 165, 229,
      133, 165, 231, 189, 145, 229, 133, 179>>},
  type => protocol, update => on_module_update}).


on_module_create(ModId, Config) ->
  ListenerCfgs =
    emqx_module_utils:parse_proto_option(ModId, Config),
  Listeners = lists:map(fun ({Proto, ListenOn,
    Options}) ->
    TabName = tabname(Proto, ListenOn),
    PredefTopics =
      parse_predefinded_topic(proplists:get_value(predefinded,
        Options,
        [])),
    {ok, RegistryPid} =
      emqx_sn_registry:start_link(TabName,
        PredefTopics),
    emqx_sn_app:start_listener({Proto, ListenOn,
      [{registry,
        {TabName,
          RegistryPid}}
        | Options]}),
    {Proto, ListenOn, RegistryPid}
                        end,
    ListenerCfgs),
  #{listeners => Listeners}.

on_module_destroy(_ModId, #{listeners := Listeners}) ->
  [begin
     emqx_sn_app:stop_listener({Proto, ListenOn, []}),
     emqx_sn_registry:stop({tabname(Proto, ListenOn),
       RegistryPid})
   end
    || {Proto, ListenOn, RegistryPid} <- Listeners],
  ok.

on_module_update(_ModId, State, Config, Config) ->
  State;
on_module_update(ModId, State, _OldConfig, Config) ->
  on_module_destroy(ModId, State),
  on_module_create(ModId, Config).

on_module_status(_ModId, #{listeners := Listeners}) ->
  Names = [listener_name(Proto, ListenOn)
    || {Proto, ListenOn, _} <- Listeners],
  case [ListenerName
    || {ListenerName, _} <- esockd:listeners(),
    lists:member(ListenerName, Names)]
  of
    [] -> #{is_alive => false};
    [_ | _] -> #{is_alive => true}
  end.

tabname(Proto, ListenOn) ->
  ListenOnStr =
    emqx_module_utils:format_listenon(ListenOn),
  list_to_atom(lists:flatten(["emqx_sn_registry_module_",
    atom_to_list(Proto), "_", ListenOnStr])).

listener_name(udp, ListenOn) ->
  {'mqttsn:udp', ListenOn};
listener_name(dtls, ListenOn) ->
  {'mqttsn:dtls', ListenOn}.

parse_predefinded_topic([]) -> [];
parse_predefinded_topic(PredefindedTopics) ->
  {Ret, _} = lists:foldl(fun (#{<<"topic">> := Topic},
      {Acc, AccLen}) ->
    {[{AccLen, Topic} | Acc], AccLen - 1}
                         end,
    {[], length(PredefindedTopics)},
    lists:reverse(PredefindedTopics)),
  Ret.
