...
-module_type(#{create => on_module_create,
  description =>
  #{en =>
  <<77, 121, 83, 81, 76, 32, 65, 85, 84, 72, 47, 65, 67,
    76>>,
    zh =>
    <<77, 121, 83, 81, 76, 32, 232, 174, 164, 232, 175,
      129, 47, 232, 174, 191, 233, 151, 174, 230, 142, 167,
      229, 136, 182>>},
  destroy => on_module_destroy,
  name => mysql_authentication,
  params =>
  #{acl_query =>
  #{default =>
  <<115, 101, 108, 101, 99, 116, 32, 97, 108, 108,
    ...
    <<115, 101, 108, 101, 99, 116, 32, 112, 97, 115,
      ...
      <<65, 117, 116, 104, 32, 81, 117, 101, 114,
        ...
        <<231, 148, 168, 230, 136, 183, 229, 144,
          141, 47, 229, 175, 134, 231, 160, 129,
          32, 83, 81, 76, 32, 232, 174, 164, 232,
          175, 129, 232, 175, 173, 229, 143,
          165>>},
      order => 8,
      title =>
      #{en =>
    ...
    <<68, 97, 116, 97, 98, 97, 115, 101, 32,
      78, 97, 109, 101, 32, 102, 111, 114, 32,
      67, 111, 110, 110, 101, 99, 116, 105,
      110, 103, 32, 116, 111, 32, 77, 121, 83,
      81, 76>>,
      zh =>
      ...
      title =>
      #{en =>
      <<77, 121, 83, 81, 76, 32, 68, 97, 116, 97,
        98, 97, 115, 101>>,
        zh =>
        <<77, 121, 83, 81, 76, 32, 230, 149, 176,
          230, 141, 174, 229, 186, 147, 229, 144,
    ...
    #{default => <<>>,
      ...
      <<80, 97, 115, 115, 119, 111, 114, 100, 32,
        ...
        101, 99, 116, 32, 116, 111, 32, 77, 121,
        83, 81, 76>>,
        zh =>
    ...
    #{default => 8,
      description =>
      #{en =>
      <<84, 104, 101, 32, 115, 105, 122, 101, 32,
        111, 102, 32, 67, 111, 110, 110, 101, 99,
        116, 105, 111, 110, 32, 80, 111, 111,
        108, 32, 102, 111, 114, 32, 77, 121, 83,
        81, 76>>,
        zh =>
        <<77, 121, 83, 81, 76, 32, 232, 191, 158,
    ...
    server =>
    #{default =>
    <<49, 50, 55, 46, 48, 46, 48, 46, 49, 58, 51, 51,
      48, 54>>,
      description =>
      #{en =>
      <<77, 121, 83, 81, 76, 32, 73, 80, 32, 65,
        100, 100, 114, 101, 115, 115, 32, 111,
        114, 32, 72, 111, 115, 116, 110, 97, 109,
        101, 32, 97, 110, 100, 32, 80, 111, 114,
        116>>,
        zh =>
...
on_module_create(ModuleId,
  ...
  _ = application:ensure_all_started(mysql),
  ...
  PoolName =
    start_resource(emqx_module_utils:pool_name(emqx_module_auth_mysql,
      ModuleId),
      Options ++ SslOptions),
  HashType =
    emqx_module_utils:password_hash(binary_to_list(maps:get(<<"password_hash">>,
      Config,
      <<"sha256">>))),
  case maps:get(<<"auth_query">>, Config, <<>>) of
    ...
    AuthQuery ->
      ...
      load_auth_hook(PoolName,
        emqx_module_utils:parse_timeout(Timeout),
        emqx_auth_mysql_cli:parse_query(binary_to_list(AuthQuery)),
        emqx_auth_mysql_cli:parse_query(SuperQuery), HashType)
  end,
  case maps:get(<<"acl_query">>, Config, <<>>) of
    ...
    AclQuery ->
      load_acl_hook(PoolName,
        emqx_module_utils:parse_timeout(Timeout),
        emqx_auth_mysql_cli:parse_query(binary_to_list(AclQuery)))
  end,
  #{<<"pool">> => PoolName}.
...
start_resource(PoolName, Options) ->
  {ok, _} = ecpool:start_sup_pool(PoolName,
    emqx_auth_mysql_cli, Options),
  PoolName.

stop_resource(PoolName) ->
  emqx:unhook('client.authenticate',
    fun emqx_auth_mysql:check/3),
...
load_auth_hook(PoolName, Timeout, AuthQuery, SuperQuery,
  ...
  emqx:hook('client.authenticate',
    fun emqx_auth_mysql:check/3, [Config]).
...
load_acl_hook(PoolName, Timeout, AclQuery) ->
  ...
  emqx:hook('client.check_acl',
    fun emqx_acl_mysql:check_acl/5, [Config]).
